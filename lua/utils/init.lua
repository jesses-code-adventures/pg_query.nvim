...
---@param mode string?
---@return file*
function M.Open_or_error(file_path, mode)
...
---@param label string
---@return string
function M.Get_query_temp_path(label)
    return M.Get_temp_dir() .. label
end
...
---@return string[]
function M.Split(value, delim, max_occurances)
    local parts = {}
    local pattern = "([^" .. delim .. "]+)"
    local idx = 1
...
function M.Run_command(command, mode)
...
---@return boolean
function M.Ensure_binary_available(binary_name)
    local success, _ = M.Run_command("which ".. binary_name)
    if not success then
